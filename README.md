...
O repositório já vem com a página e o modelo prontos. Para reproduzir do zero:

### 1. Treinar no Google Colab

Abra `treinar_mnist_colab.ipynb` no Colab. Antes de rodar, ative a GPU em **Ambiente de execução → Alterar o tipo de ambiente de execução → T4 GPU**, e então **Executar tudo**. O treino leva de 3 a 6 minutos na T4, e a última célula baixa um `entrega.zip`.

### 2. Colocar os arquivos no repositório

Descompacte o `entrega.zip` na raiz do projeto:

| Arquivo | Para quê |
|---|---|
| `modelo_web/model.json` + `.bin` | o modelo que o navegador carrega |
| `relatorio.json` | métricas do treino, lidas pelo rodapé da página |
| `modelo.keras` | o modelo original, para converter de novo sem retreinar |
...
Faça o push e, no GitHub, vá em **Settings → Pages → Source: Deploy from a branch**, escolha a branch e a pasta `/ (root)`, e **Save**.
...
├── modelo_web/                 o modelo que o navegador carrega
│   ├── model.json
│   └── group1-shard1of1.bin
├── relatorio.json              métricas do treino
├── modelo.keras                modelo original do Keras
├── relatorio/                  imagens usadas neste README
